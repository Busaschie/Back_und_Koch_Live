...
def load_users_once():
    if st.session_state.global_users is None:
        try:
            ...
            # Kopie der benötigten Spalten erstellen
            df_temp = df[["vorname", "nachname", "buchnummer"]].copy()
            # Die Buchnummer-Spalte formatieren (z.B. 12342020 -> 1234/2020)
...
with col_links:
    with st.container(border=True):
        ...
        event = st.dataframe(
            df_task_gefiltert,
            column_config={
                "shop_date": st.column_config.DateColumn(
                    "Einkaufsdatum",
                    format="DD.MM.YYYY"  # Zeigt das Datum als DD.MM.YYYY an
                )
            },
            hide_index=True,
            width="stretch",
    ...
    with st.container(border=True):
        ...
        with st.expander("Schritt 4: Abbuchung"):
            ...
            with st.form("abbuchung_form"):
                ...
                # Spaltenkonfiguration mit Großbuchstaben für die Headers
                if ist_buchung_gespeichert:
                    spalten_konfiguration_4 = {
                        "vorname": st.column_config.TextColumn("VORNAME"),
                        "nachname": st.column_config.TextColumn("NACHNAME"),
                        "buchnummer": st.column_config.TextColumn("BUCHNUMMER"),
                        "aktuelles_guthaben": None,  # Ausgeblendet lassen
            ...
            if submitted_4:
                gueltige_abbuchungen = df_editiert_4[df_editiert_4["abbubuchung" if "abbubuchung" in df_editiert_4 else "abbuchung"] > 0.0]
                    ...
                    if erfolgreich > 0:
                        try:
                            ...
                            if status_response.status_code in [200, 201]:
                                ...
                                st.warning(f"Status-Update failed: {status_response.text}")
                        except Exception as e:
